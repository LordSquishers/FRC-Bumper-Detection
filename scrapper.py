...
def section(info_dict, ydl):
    ...
    if length and length > 60:
        ...
        return [{"start_time": t, "end_time": t+30}]
...
events = json.loads(requests.get(baseUrl + f"/events/2024vafal/keys", headers=headers).content)
a =1
keys = []
for event in events:
    a+=1
    matches = json.loads(requests.get(f"{baseUrl}/event/2024vafal/matches", headers=headers).text)
